...
function PromptPill({ text }: { text: string }) {
  ...
  return (
    ...
    >
      <span className="text-sm text-zinc-300 font-mono">&quot;{text}&quot;</span>
    ...
    >
      ...
      >
        <Check
          className={cn(
            "w-3 h-3",
            checked ? "text-black" : "text-zinc-600",
          )}
        />
      </div>
      <div>
...
export function Onboarding({
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const res = await fetch(`/api/logs?streamId=${detectedStreamId}&limit=1`);
        if (!res.ok) return;
  ...
  return (
    <div className="flex items-start gap-5 w-full max-w-2xl mx-auto">
      ...
      {/* Card */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex-1">
        ...
        {step === 2 && (
          <>
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-400">
                Open your AI tool and ask it this — it should confirm Loghead
                tools are available.
              </p>
            </div>
        ...
        {step === 3 && (
          <>
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-400">
                Pick a stream type, then copy the prompt and paste it into your
                AI tool — it will set everything up.
              </p>
            </div>
        ...
        {step === 4 && (
          <>
            ...
            {streamType === "browser" ? (
              <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 space-y-3">
                <p className="text-sm text-zinc-300">
                  ...
                  <span className="text-emerald-400 font-medium">Loghead Chrome Extension</span>{" "}
            ...
            >
              ...
              <div>
                <div className={cn("text-sm font-medium", logsFound ? "text-emerald-300" : "text-zinc-400")}>
                  {logsFound ? "Logs are flowing" : "Waiting for first log…"}
                </div>
                <div className="text-xs text-zinc-500 mt-0.5">
                  {logsFound
        ...
        {step === 5 && (
          <>
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-400">
                Logs are flowing into Loghead. Use these prompts to start querying them with your AI tool.
              </p>
            </div>
