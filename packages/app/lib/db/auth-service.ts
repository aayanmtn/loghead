...
export class AuthService {
  ...
  async getOrCreateMcpToken(): Promise<string> {
    ...
    const row = await this.db.get<{ value: string }>(
      "SELECT value FROM system_config WHERE key = 'mcp_token'",
    );
    if (row?.value) {
      const decoded = AuthService.decodeTokenUnsafe(row.value) as {
        tenantId?: string;
      } | null;

      // Reuse existing token only if it matches current tenant scope (or no tenant scope requested).
      if (!this.tenantId || decoded?.tenantId === this.tenantId) {
        return row.value;
      }
    }
    ...
    const payload: any = { sub: "system:mcp", iss: "loghead", role: "admin" };
    if (this.tenantId) {
    ...
    if (row?.value) {
      await this.db.run(
        "UPDATE system_config SET value = ? WHERE key = 'mcp_token'",
        [token],
      );
    } else {
      await this.db.run(
        "INSERT INTO system_config (key, value) VALUES ('mcp_token', ?)",
        [token],
      );
    }
  ...
  async verifyToken(
    token: string,
  ): Promise<{ streamId: string; role?: string } | null> {
    await this.initialize();
    if (!this.secretKey) throw new Error("Auth not initialized");
