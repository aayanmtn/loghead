  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.storage.local.get(["serverUrl", configKey], (result) => {
      if (result.serverUrl) {
        ...
        serverUrlInput.value = "https://loghead.dev";
      }
    ...
    nextBtn.addEventListener("click", async () => {
      ...
      try {
        const projects = await mcpRequest(
    ...
    projectSelect.addEventListener("change", async () => {
      ...
      try {
        const streams = await mcpRequest(
          serverUrlInput.value,
          "streams",
          { projectId },
          "GET",
        ...
        streamSelect.innerHTML =
          '<option value="" disabled selected>Select a Browser Stream</option><option value="new">-- Create New Stream --</option>';
    ...
    createStreamBtn.addEventListener("click", async () => {
      ...
      try {
        const newStream = await mcpRequest(serverUrlInput.value, "streams", {
          projectId,
          name,
          type: "browser",
        });
