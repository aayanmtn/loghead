...
export class DbService {
    ...
    async listProjects(): Promise<Project[]> {
        try {
            const projects = await this.db.query<Project>("SELECT * FROM projects ORDER BY created_at DESC");
            ...
            const projectsWithStreams = await Promise.all(projects.map(async (p) => {
                const streams = await this.db.query<Stream>("SELECT * FROM data_streams WHERE project_id = ?", [p.id]);
                return { ...p, streams };
            }));
    ...
    async getStream(id: string): Promise<Stream | undefined> {
        const stream = await this.db.get<Stream>("SELECT * FROM data_streams WHERE id = ?", [id]);
    ...
    async listStreams(projectId: string): Promise<Stream[]> {
        const streams = await this.db.query<Stream>("SELECT * FROM data_streams WHERE project_id = ? ORDER BY created_at DESC", [projectId]);
    ...
    async addLog(streamId: string, content: string, metadata: Record<string, unknown> = {}): Promise<{ id: string }> {
        ...
        await this.db.transaction(async () => {
            ...
            if (rowid === undefined) {
                const inserted = await this.db.get<{ rowid: number | bigint }>("SELECT rowid FROM logs WHERE id = ?", [id]);
