export async function migrate(db, verbose = true) {
  ...
  `);
    ...
    try {
        const vecTableInfo = await db.query("PRAGMA table_info(vec_logs)");
  ...
  `);
    ...
    if (verbose) {
        console.warn("[Vector] Skipping vec_logs_idx creation in local embedded mode; vector search will use fallback path.");
    }
    if (verbose)
